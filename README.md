...
![The Idle Cup overview](docs/images/the-idle-cup-overview.png)
...
<sub>本リポジトリは AWS Summit Japan 2026 AI-DLC ハッカソン応募作品として公開されています。`awslabs/aidlc-workflows` v0.1.8 の**公式階層に準拠**（運用面の Mob 合議は Phase 1 未達。詳細は [`aidlc-docs/extensions/meta/methodology-honesty.md`](aidlc-docs/extensions/meta/methodology-honesty.md) §3 主張1 の準拠領域マトリクス）。独自拡張は **Phase 1 配置** が Self-Reference / Methodology-Honesty / Business-Intent / Demo-Scenario の 4 件、**Phase 2 配置予定** が Living-Spec、**audit.md を SoT とする運用** が Bolt-Logging（`aidlc-docs/extensions/meta/bolts/` は Phase 2 セッションログ用にディレクトリのみ確保）。コーヒー豆のデータについては、Phase 1 提出物では一般名称・引用範疇に留めており、Phase 2 で実データ提携を行う際は焙煎所と書面合意を交わす方針です（詳細: [`aidlc-docs/extensions/business/business-intent.md`](aidlc-docs/extensions/business/business-intent.md) §7 リスク表）。Amazon SP-API、Amazon Pay、Amazon Alexa、FBA との統合は、Phase 2 で実装する設計済みの拡張領域です。</sub>
