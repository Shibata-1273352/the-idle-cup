...
| 「焙煎所の豆データの著作権・利用規約」 | Phase 1 提出物に登場する銘柄名・産地・処理は一般名称・引用範疇に留め、固有のテイスティングノートやマーケティング文の転載はしない。Phase 2 で実データ提携時には焙煎所と書面合意を交わし、`operations/data-licensing.md`（Phase 2 で配置）に契約スコープを記録する |
