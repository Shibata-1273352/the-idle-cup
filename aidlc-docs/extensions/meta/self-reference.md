...
> **配置根拠**: 公式 v0.1.8 では meta 文書は明示の場を持たない。本作品の主張上不可欠なため `extensions/meta/` に独自配置。
...
この同型性は本プロジェクトの差別化の核として機能することを意図しており、特にハッカソンの審査基準のうち「創造性とテーマ適合性」に対する応答として位置づけている（最終的な評価は審査員に委ねる）。
